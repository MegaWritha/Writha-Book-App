...
export default function ReaderScreen() {
  ...
  // FIXED: Type assertion to handle the string | string[] error from useLocalSearchParams
  ...
  const startHideTimer = () => {
    ...
    hideTimeout.current = setTimeout(() => {
      if (showControls) fadeControls(0);
    }, 4500); 
  };
  ...
  const fadeControls = (toValue: number) => {
    Animated.timing(fadeAnim, {
      ...
      if (toValue === 0) setShowControls(false);
    });
    if (toValue === 1) setShowControls(true);
  };

  const userInteracted = () => {
    if (!showControls) fadeControls(1);
    startHideTimer();
  };
    ...
    const loadGoal = async () => {
      ...
      try {
        ...
        if (savedGoal) {
          ...
          // Only update if current value is actually different to prevent re-render loops
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      if (isOfflineMode) {
        try {
          // FINAL FIX: Using bracket notation to bypass the TypeScript property check
          // and providing a fallback for web/environments where documentDirectory is null
          const baseDir = (FileSystem as any)['documentDirectory'] || (FileSystem as any)['cacheDirectory'];
          
          if (!baseDir) {
             // Fallback for Web/Testing environments
             processBookData({ title: "Offline Preview", content: "Offline mode not supported on web." });
             return;
          }

          const mDir = `${baseDir}manuscripts/`;
          const dirInfo = await FileSystem.getInfoAsync(mDir);
          if (!dirInfo.exists) {
          ...
          processBookData(offlineData);
        } catch (error) {
          console.error("FS Error:", error);
          Alert.alert("Archive Error", "Could not locate manuscript in local storage.");
          router.back();
        }
      } else {
  ...
  const handleScroll = (event: any) => {
    ...
    const cleanProgress = Math.max(0, Math.min(100, rawProgress));
    setProgress(cleanProgress);

    const pagesRead = Math.floor(cleanProgress / 10);
    // Only update and persist if progress has actually increased
    if (pagesRead > readingGoal.current) {
      const newGoal = { ...readingGoal, current: pagesRead };
      setReadingGoal(newGoal);
      AsyncStorage.setItem(`readingGoal_${bookId}`, JSON.stringify(newGoal)).catch(err => console.log(err));
    }
  ...
  const handleZoneClick = (direction: 'left' | 'right') => {
    ...
    if (readingMode === 'scroll') {
      const move = direction === 'left' ? -height * 0.7 : height * 0.7;
      scrollRef.current?.scrollTo({ y: scrollPos + move, animated: true });
    } else {
      const move = direction === 'left' ? -width : width;
      scrollRef.current?.scrollTo({ x: scrollPos + move, animated: true });
    }
  };
    ...
    if (readingMode === 'swipe') {
        ...
        return pages.map((page, pageIdx) => (
            <View key={`page-${pageIdx}`} style={{ width: width, paddingHorizontal: 32 }}>
                {page.map((line, i) => renderLine(line, `p-${pageIdx}-${i}`))}
            </View>
        ));
    }
    return paragraphs.map((line, i) => renderLine(line, `line-${i}`));
  };
  ...
  return (
    <View style={[styles.mainContainer, { backgroundColor: theme.bg }]}>
      ...
      <Pressable style={styles.zoneLeft} onPress={() => handleZoneClick('left')} />
      <Pressable style={styles.zoneRight} onPress={() => handleZoneClick('right')} />

      {/* TOP NAV */}
      <Animated.View style={[styles.topPanel, { backgroundColor: theme.ui, opacity: fadeAnim, transform: [{ translateY: fadeAnim.interpolate({ inputRange: [0, 1], outputRange: [-150, 0] }) }] }]}>
        <View style={styles.topActionRow}>
          ...
          <View style={styles.titleContainer}>
            ...
            {isOfflineMode && <MaterialCommunityIcons name="cloud-check" size={12} color={theme.accent} />}
          </View>
          <TouchableOpacity style={styles.iconBox} onPress={() => { userInteracted(); router.push(`/book/${bookId}/dictionary`); }}>
        ...
        <View style={styles.goalBanner}>
            <Text style={[styles.goalText, { color: theme.accent }]}>
              DAILY GOAL: {readingGoal.current}/{readingGoal.target} PAGES
            </Text>
        </View>

        <View style={styles.progressContainer}>
          <View style={[styles.progressTrack, { backgroundColor: theme.isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)' }]} />
      ...
      <ScrollView 
        ...
        horizontal={readingMode === 'swipe'}
        pagingEnabled={readingMode === 'swipe'}
        showsVerticalScrollIndicator={false}
        showsHorizontalScrollIndicator={false}
      >
        <Pressable onPress={userInteracted}>
          <View style={[styles.manuscriptHeader, readingMode === 'swipe' && { width: width }]}>
            {book?.displayCover ? (
              ...
              <View style={[styles.placeholderCover, { backgroundColor: theme.ui }]}>
                <MaterialCommunityIcons name="book-open-page-variant" size={80} color={theme.accent} />
              </View>
            )}
          ...
          {readingMode === 'scroll' && <View style={{ height: 450 }} />}
        </Pressable>
      </ScrollView>

      {/* BOTTOM PANEL */}
      <Animated.View style={[styles.bottomPanel, { backgroundColor: theme.ui, opacity: fadeAnim, transform: [{ translateY: fadeAnim.interpolate({ inputRange: [0, 1], outputRange: [500, 0] }) }] }]}>
        <View style={styles.navTabs}>
          <TouchableOpacity style={styles.tabItem} onPress={() => { userInteracted(); router.push(`/book/${bookId}/findings`); }}>
            <Feather name="search" size={24} color={theme.accent} />
            <Text style={[styles.tabLabel, { color: theme.text }]}>FINDINGS</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.tabItem} onPress={() => { userInteracted(); setReadingMode(readingMode === 'scroll' ? 'swipe' : 'scroll'); }}>
            <MaterialCommunityIcons name={readingMode === 'scroll' ? "unfold-more-horizontal" : "unfold-more-vertical"} size={24} color={theme.accent} />
            <Text style={[styles.tabLabel, { color: theme.text }]}>{readingMode === 'scroll' ? 'SWIPE' : 'SCROLL'}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.tabItem} onPress={() => { userInteracted(); router.push({ pathname: "/book/[id]/chapters", params: { id: bookId } }); }}>
            <MaterialCommunityIcons name="format-list-bulleted" size={24} color={theme.accent} />
            <Text style={[styles.tabLabel, { color: theme.text }]}>CHAPTERS</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.tabItem} onPress={toggleSpeech}>
            <FontAwesome5 name={isSpeaking ? "stop-circle" : "headphones-alt"} size={22} color={isSpeaking ? "#FF4444" : theme.accent} />
            <Text style={[styles.tabLabel, { color: isSpeaking ? "#FF4444" : theme.text }]}>VOICE</Text>
          </TouchableOpacity>
        </View>

        <View style={styles.controlSection}>
          <TouchableOpacity style={styles.sizeBtn} onPress={() => { userInteracted(); setFontSize(s => Math.max(12, s-1)); }}><Text style={{ color: theme.text, fontSize: 16 }}>A-</Text></TouchableOpacity>
          <View style={[styles.alignmentRow, { borderColor: theme.accent + '40' }]}>
            <TouchableOpacity onPress={() => { userInteracted(); setAlignment('left'); }}><MaterialCommunityIcons name="format-align-left" size={22} color={alignment === 'left' ? theme.accent : theme.text} /></TouchableOpacity>
            <TouchableOpacity onPress={() => { userInteracted(); setAlignment('center'); }}><MaterialCommunityIcons name="format-align-center" size={22} color={alignment === 'center' ? theme.accent : theme.text} /></TouchableOpacity>
            <TouchableOpacity onPress={() => { userInteracted(); setAlignment('justify'); }}><MaterialCommunityIcons name="format-align-justify" size={22} color={alignment === 'justify' ? theme.accent : theme.text} /></TouchableOpacity>
          </View>
          <TouchableOpacity style={styles.sizeBtn} onPress={() => { userInteracted(); setFontSize(s => Math.min(40, s+1)); }}><Text style={{ color: theme.text, fontSize: 20 }}>A+</Text></TouchableOpacity>
        </View>

        <View style={styles.footerActions}>
          <View style={styles.themePalette}>
            {['void', 'sepia', 'slate', 'paper'].map((t) => (
...
const styles = StyleSheet.create({
  ...
  zoneLeft: { position: 'absolute', left: 0, top: 0, bottom: 0, width: '25%', zIndex: 10 },
  zoneRight: { position: 'absolute', right: 0, top: 0, bottom: 0, width: '25%', zIndex: 10 },
  ...
  titleContainer: { flex: 1, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 5 },
  ...
  progressContainer: { width: '100%', height: 6, position: 'absolute', bottom: 0 },
  progressTrack: { width: '100%', height: '100%', opacity: 0.2 },
