...
import {
  ...
  deleteDoc, where, updateDoc, getDoc,
  ...
  useEffect(() => {
    ...
    const unsubProfile = onSnapshot(doc(db, "users", user.uid), (snap) => {
      if (snap.exists()) {
        const data = snap.data();
        setProfile(data);
        // ✅ Check admin from profile snapshot
    ...
    const unsubLikes = onSnapshot(collection(db, "users", user.uid, "likedBooks"), async (snap) => {
      ...
      const bookDetails = await Promise.all(ids.map(async (id) => {
        try {
          ...
          return { id, title: b.title, cover: `https://www.gutenberg.org/cache/epub/${id}/pg${id}.cover.medium.jpg` };
        } catch { return null; }
      }));
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.headerWrapper}>
        ...
        <View style={styles.avatarWrap}>
          <View style={styles.premiumAvatarBorder}>
            <Image
              source={{ uri: profile?.profilePic || `https://ui-avatars.com/api/?name=${profile?.firstName}` }}
              style={styles.avatar}
            />
      ...
      <View style={styles.identity}>
        ...
        {isAdmin && (
          ...
          >
            <MaterialCommunityIcons name="shield-crown" size={18} color="#000" />
...
const styles = StyleSheet.create({
  ...
  adminBtn: { flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: "#FFD700", paddingHorizontal: 28, paddingVertical: 12, borderRadius: 25, marginTop: 14 },
  adminBtnTxt: { color: "#000", fontWeight: "900", fontSize: 13, letterSpacing: 1 },
